...
function filtrarSeriesPorAno(series, ano) {
	var seriesPorAno = [];
	series.filter(a => { if(a.anoEstreia >= ano) seriesPorAno.push(a) });
	return seriesPorAno;
...
function mediaDeEpisodios(series) {
		var resultado = 0;
		series.forEach(a => { resultado += a.numeroEpisodios })
...
function procurarPorNome(series, nome) {
	var resposta = false;
	series.forEach(a => { a.elenco.forEach(b => { if(b.includes(nome)) resposta = true}) } );
	return resposta;
}
...
function queroGenero(generoFornecido) {
	var arrayGenero = [];
	series.forEach(a => a.genero.forEach( b => { if (generoFornecido === b) arrayGenero.push(a.titulo)})); 
	return arrayGenero;
}
...
function queroTitulo(parteTitulo) {
	var arrayFilmes = [];
	series.forEach(a => { if (a.titulo.includes(parteTitulo)) arrayFilmes.push(a.titulo)}); 
	return arrayFilmes;
}
...
function creditosIlluminatis(n) {
	...
	const diretor = n.diretor.map(a => a.replace(/(\w+)\s(\w+)/, '$2, $1')).sort().join("\n");
	const elenco = n.elenco.map(a => a.replace(/(\w+)\s(\w+)/, '$2, $1')).sort().join("\n");
...
function sobrenomeAbreviado(series) {
	...
	series.forEach(
		a => a.elenco.forEach( 
			b => { if( (b[(b.indexOf(" ") + 2)]) !== ".") resposta = false;  })
		if(resposta === true) serieAbreviada = a.titulo break );
